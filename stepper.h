...
class Cstepper_NewPortXYZ: public Cstepper
{
protected:
  //! communication with stepper
  Cserial* pComStepper;
public:
  ...
  **/
  Cstepper_NewPortXYZ()
  {
    ///set class name (debug only)
#if cimg_debug>1
    class_name="Cstepper_NewPortXYZ";
#endif
    ///set library version (information only)
    class_version=STEPPER_VERSION;
    ///set axis names
...
std::cerr<<class_name<<"::"<<__func__<<"("<<stepper_port_path<<","<<stepper_port_type<<")\n"<<std::flush;
  ...
  //! \todo remove valueToString function should be in parent class !
  //! convert value to string
  /**
   * format value to string
  **/
